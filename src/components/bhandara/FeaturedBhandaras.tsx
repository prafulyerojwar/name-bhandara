...
// Demo data is built inside useEffect so Date.now() only ever runs
// on the client — never on the server during SSR prerender.
function buildDemos(): Bhandara[] {
  const now = Date.now();
  ...
  return [
    ...
    {
      id: 'demo2',
      title: 'Wedding Reception Surplus Food',
...
export default function FeaturedBhandaras() {
  const [demos, setDemos] = useState<Bhandara[]>([]);
    ...
    <section className="py-20 bg-gradient-to-b from-orange-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          ...
          {/* Skeleton placeholders while demos load on client */}
          {demos.length === 0 && Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-white rounded-3xl border border-orange-100 shadow-md h-64 animate-pulse" />
          ))}
        </div>
